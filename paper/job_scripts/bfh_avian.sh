...
#SBATCH --job-name="run_bfh_avian" # a nice readable name to give your job so you know what it is when you see it in the queue, instead of just numbers
...
do
  ...
  do
    echo $k : $(date)
    command time -f "CPU:%P  MEM(kbytes):%M" python3 BipartitionFrequencyHash.py avian.48.$k avian.48.$k $n $cpu &> avian.48.$k.bfh.$cpu.log
  done
done