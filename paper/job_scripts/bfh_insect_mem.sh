...
#SBATCH --job-name="m_insect" # a nice readable name to give your job so you know what it is when you see it in the queue, instead of just numbers
...
do
  ...
  do
    echo $k : $(date)
    command time -f "CPU:%P  MEM(kbytes):%M" python3 BipartitionFrequencyHash.py insect.144.$k insect.144.$k $n $cpu &> insect.144.$k.bfh.$cpu.mem.log
  done
done