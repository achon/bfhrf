...
#SBATCH --job-name="m_k" # a nice readable name to give your job so you know what it is when you see it in the queue, instead of just numbers
...
do
  ...
  do
    echo $k : $(date)
    command time -f "CPU:%P  MEM(kbytes):%M" python3 BipartitionFrequencyHash.py gene_trees.1.99.$k gene_trees.1.99.$k $n $cpu &> gene_trees.1.99.$k.bfh.$cpu.mem.log
  done
done
