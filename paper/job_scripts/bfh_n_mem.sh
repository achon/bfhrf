...
#SBATCH --job-name="m_n" # a nice readable name to give your job so you know what it is when you see it in the queue, instead of just numbers
...
do
  ...
  do
    ((m=n+1))
    echo "$m" : $(date)
    command time -f "CPU:%P  MEM(kbytes):%M" python3 BipartitionFrequencyHash.py gene_trees.1.$n.$k gene_trees.1.$n.$k "$m" $cpu &> gene_trees.1.$n.$k.bfh.$cpu.mem.log
  done
done
